...
class flyAshSold extends StatefulWidget {
  const flyAshSold({super.key});

  @override
  State<flyAshSold> createState() => _flyAshSoldState();
}

class _flyAshSoldState extends State<flyAshSold> {
  ...
  fatchichDatabaseList() async {
    ...
    var inter = await FirebaseFirestore.instance.collection('fly ash Bricks').doc('add');
    inter.get().then((inter){
      if(inter['amount'] == null)
      {
        setState(() {
          interAmount = 0;
        });
