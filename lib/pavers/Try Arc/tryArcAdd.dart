...
class tryArcAdd extends StatefulWidget {
  const tryArcAdd({super.key});

  @override
  State<tryArcAdd> createState() => _tryArcAddState();
}

class _tryArcAddState extends State<tryArcAdd> {

  CollectionReference? fileRef = FirebaseFirestore.instance.collection('Try Arc Pavers');
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
                                          ...
                                          children: [
                                            ...
                                            TextButton(
                                                onPressed: () async {
                                                  ...
                                                  await FirebaseFirestore.instance.collection('Try Arc Pavers').doc(color).set({
                                                    'color' : color,
                                                    'amount' : int.tryParse(amount),
                                                  });
                                                },
      ...
      child: Container(
        ...
        child: const Center(child: Text("Try Arc",style: TextStyle(fontSize: 19,color: Colors.black),)),
      ),
    );
  }
}
