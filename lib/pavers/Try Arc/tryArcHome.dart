...
class tryArcHome extends StatefulWidget {

  final String url;

  const tryArcHome({required this.url,super.key});

  @override
  State<tryArcHome> createState() => _tryArcHomeState();
}

class _tryArcHomeState extends State<tryArcHome> {
  ...
  String name = 'Try Arc Pavers';

  CollectionReference? dataRef = FirebaseFirestore.instance.collection('pavers');

  fetchValue() async {

    var _value = await FirebaseFirestore.instance.collection('pavers').doc('Try Arc Pavers');
