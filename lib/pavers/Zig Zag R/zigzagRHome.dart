...
class zigzagRHome extends StatefulWidget {

  final String url;

  const zigzagRHome({required this.url,super.key});

  @override
  State<zigzagRHome> createState() => _zigzagRHomeState();
}

class _zigzagRHomeState extends State<zigzagRHome> {
  ...
  int value = 0;
  String name = 'Zig Zag R Pavers';

  CollectionReference? dataRef = FirebaseFirestore.instance.collection('pavers');

  fetchValue() async {

    var _value = await FirebaseFirestore.instance.collection('pavers').doc(name);
