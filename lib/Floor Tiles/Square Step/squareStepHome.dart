...
class SquareStepHome extends StatefulWidget {

  final String url;

  const SquareStepHome({required this.url,super.key});

  @override
  State<SquareStepHome> createState() => _SquareStepHomeState();
}

class _SquareStepHomeState extends State<SquareStepHome> {

  bool isTapped = true;
  bool isExpanded = false;

  CollectionReference? fileRef = FirebaseFirestore.instance.collection('Square Step Floor Tiles');
  ...
  int value = 0;
  String name = 'Square Step Floor Tiles';

  CollectionReference? dataRef = FirebaseFirestore.instance.collection('floor tiles');

  fetchValue() async {

    var _value = await FirebaseFirestore.instance.collection('floor tiles').doc(name);
